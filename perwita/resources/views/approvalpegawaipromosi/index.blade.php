...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8">
        <h2>Approval Promosi Pegawai</h2>
        <ol class="breadcrumb">
            <li>
                <a href="{{ url('/') }}">Home</a>
            </li>
            <li class="active">
                <strong>Daftar Approval Promosi Pegawai</strong>
            </li>
        </ol>
    </div>
</div>
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="ibox-title ibox-info">
        <h5>Daftar Approval Promosi Pegawai</h5>
    </div>
    <div class="ibox">
        <div class="ibox-content">
            <div class="row m-b-lg">
                ...
                <form class="formapprovalpromosi" id="formapprovalpromosi">
                  @foreach($data as $z)
                  <input type="hidden" name="ppd_no" id="ppd_no" value="{{$z->ppd_no}}">
                  @endforeach
                    <table id="promositabel" class="table table-bordered table-striped" >
                        ...
                        <tbody>
                          ...
                          <tr class="select-{{$index}}" onclick="select({{$index}})" style="cursor: pointer;">
                              <td>
                                  <input class="pilih-{{$index}}" type="checkbox" name="pilih[]" onclick="selectBox({{$index}})" value="{{$x->ppd_id}}">
                              </td>
                                <td>{{$x->p_name}}</td>
                                <td>{{$x->ppd_jabatan_awal}}</td>
                                <td>{{$x->ppd_jabatan_sekarang}}</td>
                                <td>{{$x->p_nip}}</td>
                                @if(stristr($x->ppd_no, 'PMS'))
                                <td class="sweetdinamis{{$x->ppd_id}}">Promosi</td>
                                @elseif(stristr($x->ppd_no, 'DMS'))
                                <td class="sweetdinamis{{$x->ppd_id}}">Demosi</td>
                                @endif
                                <td align="center">
                                <button type="button" title="Detail" onclick="detail({{$x->ppd_id}})" id="detailbtn" class="btn btn-info btn-sm" name="button"> <i class="glyphicon glyphicon-folder-open"></i> </button>
                                <button type="button" title="Setujui" onclick="setujui({{$x->ppd_id}})" class="btn btn-primary btn-sm" name="button"> <i class="glyphicon glyphicon-ok"></i> </button>
                                <button type="button" title="Tolak" onclick="tolak({{$x->ppd_id}})"  class="btn btn-danger btn-sm" name="button"> <i class="glyphicon glyphicon-remove"></i> </button>
                               </td>
                            </tr>
                            @endforeach
                        </tbody>
  ...
  swal({
      ...
      function () {
          ...
          setTimeout(function () {
              $.ajax({
                type: 'get',
                data: {id:id, ppd_no:ppd_no},
                url: baseUrl + '/approvalpegawaipromosi/setujui',
                dataType: 'json',
                timeout: 10000,
      ...
      function () {
          ...
          setTimeout(function () {
              $.ajax({
                type: 'get',
                data: {id:id},
                url: baseUrl + '/approvalpegawaipromosi/tolak',
                dataType: 'json',
                timeout: 10000,
                  success: function (response) {
                      waitingDialog.hide();
...
function setujuilist(){
  ...
  $.ajax({
    ...
    url: baseUrl + '/approvalpegawaipromosi/setujuilist',
    dataType: 'json',
    timeout: 10000,
...
function tolaklist(){
  ...
  $.ajax({
    type: 'get',
    data: $('#formapprovalpromosi').serialize(),
    url: baseUrl + '/approvalpegawaipromosi/tolaklist',
    dataType: 'json',
    timeout: 10000,
    success : function(result){
      waitingDialog.hide();
...
function detail(id){
  ...
  $.ajax({
    type: 'get',
    data: {id:id},
    url: baseUrl + '/approvalpegawaipromosi/detail',
    dataType: 'json',
    success : function(result){

      $('#jabatanawal').text(result.ppd_jabatan_awal);
      $('#jabatansekarang').text(result.ppd_jabatan_sekarang);
      $('#pdno').text(result.ppd_no);
      $('#namapekerja').text(result.ppd_pegawai);
      $('#keterangan').text(result.ppd_note);
