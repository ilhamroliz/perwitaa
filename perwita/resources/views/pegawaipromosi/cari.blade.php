...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8">
        <h2>Cari Promosi & Demosi Pegawai</h2>
        <ol class="breadcrumb">
            <li>
                <a href="{{ url('/') }}">Home</a>
            </li>
            ...
            <li class="active">
                <strong>Cari Promosi & Demosi Pegawai</strong>
            </li>
        </ol>
    </div>
</div>
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="ibox-title">
        <h5>Cari Promosi & Demosi Pegawai</h5>
      <a href="{{url('manajemen-pegawai/promosidemosi')}}" style="float: right; margin-top: -7px;" class="btn btn-outline btn-primary btn-flat"><i class="fa fa-plus">&nbsp;</i>Tambah</a>
    </div>
    <div class="ibox">
                      ...
                      <div class="form-group">
                          ...
                          <div class="col-lg-8">
                              <select class="form-control jabatan" name="jabatanBaru" id="jabatanbaru">
                                  ...
                                  <option value="{{ $jabatan->j_id }}" id="{{ $jabatan->j_id }}">{{ $jabatan->j_name }}</option>
                                  @endforeach
                              </select>
                          </div>
                      </div>
...
$(document).ready(function(){
  ...
  processing: true,
  serverSide: true,
  ajax: '{{ url('manajemen-pegawai/promosidemosi/tabelcari') }}',
  dataType: 'json',
  columns: [
      {data: 'ppd_no', name: 'ppd_no'},
  ...
  "language": dataTableLanguage,
    ...
    $('#pencarian').autocomplete({
        source: baseUrl + '/manajemen-pegawai/promosidemosi/getno',
...
//
  function getdata(id){
    ...
    $.ajax({
      type: 'get',
      data: {id:id},
      url: baseUrl + '/manajemen-pegawai/promosidemosi/getdata',
      dataType: 'json',
      success : function(result){
        if (result.status == 'kosong') {
          ...
          for (var i = 0; i < result.length; i++) {
            html += '<tr>'+
                    '<td>'+result[i].ppd_no+'</td>'+
                    '<td>'+result[i].p_name+'</td>'+
                    '<td>'+result[i].ppd_jabatan_awal+'</td>'+
                    '<td>'+result[i].ppd_jabatan_sekarang+'</td>'+
                    '<td>'+result[i].p_nip+'</td>'+
                    '<td>'+
                    '<div class="text-center">'+
                      '<a style="margin-left:5px;" title="Detail" type="button" onclick="detail('+result[i].ppd_id+')"  class="btn btn-info btn-xs"><i class="glyphicon glyphicon-folder-open"></i></a>'+
                      '<a style="margin-left:5px;" title="Edit" type="button" onclick="edit('+result[i].ppd_id+')"  class="btn btn-warning btn-xs"><i class="glyphicon glyphicon-edit"></i></a>'+
                      '<a style="margin-left:5px;" title="Hapus" type="button" onclick="hapus('+result[i].ppd_id+')"  class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-trash"></i></a>'+
                    '</div>'+
                    '</tr>';
          }
        }
  ...
  function detail(id){
    ...
    $.ajax({
      type: 'get',
      data: {id:id},
      url: baseUrl + '/manajemen-pegawai/promosidemosi/detail',
      dataType: 'json',
      success : function(result){

        $('#pd_no').text(result[0][0].ppd_no);
        $('#namapekerja').text(result[0][0].p_name);
        $('#keteranganpekerja').text(result[0][0].ppd_note);
        ...
        } else if (result[0][0].ppd_isapproved == 'N') {
    ...
    function(){
      ...
      waitingDialog.show();
        setTimeout(function(){
          $.ajax({
            data: {id:id},
            url: baseUrl + '/manajemen-pegawai/promosidemosi/hapus',
            type: 'get',
            timeout: 10000,
  ...
  function edit(id){
    ...
    $.ajax({
      type: 'get',
      data: {id:id},
      url: baseUrl + '/manajemen-pegawai/promosidemosi/edit',
      dataType: 'json',
      success : function(result){
        $('div.col-lg-8 select').val(result[0].ppd_jabatan_sekarang);
        $('#note').val(result[0].ppd_note);
        // button
  ...
  function update(id){
    setTimeout(function(){
      ...
      $.ajax({
        ...
        url: baseUrl + '/manajemen-pegawai/promosidemosi/update/'+id,
        dataType: 'json',
        timeout: 10000,
        success : function(result){
