...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">
                <strong>Daftar Approval SP</strong>
            </li>
        </ol>
    </div>
</div>
                    ...
                    <table id="tabel-sp" class="table table-bordered table-striped" >
                        ...
                        <tbody>
                          ...
                          <tr class="select-{{$index}}" onclick="select({{$index}})" style="cursor: pointer;">
                              <td>
                                  <input class="pilih-{{$index}}" type="checkbox" name="pilih[]" onclick="selectBox({{$index}})" value="{{$x->sp_id}}">
                              </td>
                              <td>{{$x->sp_no}}</td>
                              <td>{{$x->p_name}}</td>
                              <td>{{$x->p_jabatan}}</td>
...
function setujui(id){
  swal({
          title: "Konfirmasi",
          text: "Apakah anda yakin ingin menyetujui SP ini?",
          type: "warning",
      ...
      function () {
          ...
          setTimeout(function () {
              $.ajax({
                type: 'get',
                data: {id:id},
                url: baseUrl + '/approvalsp/setujui',
                dataType: 'json',
                timeout: 10000,
...
function tolak(id){
  swal({
          title: "Konfirmasi",
          text: "Apakah anda yakin ingin menolak SP ini?",
          type: "warning",
          showCancelButton: true,
          closeOnConfirm: false,
          showLoaderOnConfirm: true,
...
function setujuilist(){
  ...
  $.ajax({
    ...
    url: baseUrl + '/approvalsp/setujuilist',
    dataType: 'json',
    timeout: 10000,
...
function tolaklist(){
  ...
  $.ajax({
    type: 'get',
    data: $('#formapprovalsp').serialize(),
    url: baseUrl + '/approvalsp/tolaklist',
    dataType: 'json',
    timeout: 10000,
    success : function(result){
      waitingDialog.hide();
